...
const TITLE: &str = "niinii";

pub fn main_loop<F>(mut run_ui: F)
where
    F: FnMut(&mut bool, &mut Env, &mut Ui),
{
    ...
    let (mut event_loop, window) = create_window();
...
fn imgui_init(window: &Window) -> (WinitPlatform, imgui::Context, Env) {
